...
class BenchmarkCalculator(Calculator):
    ...
    def calculate(self, atoms=None, properties=["energy", "forces"], system_changes=all_changes, measure_time=False):
        ...
        energy, forces = first_model(batch)
        energy_first_model = energy.detach().cpu().item()
        forces_first_model = forces.detach().cpu().numpy()

        # 여러 모델로 에너지 예측 수행 (uncertainty 계산)
        ...
        # 결과 저장: 첫 번째 모델의 에너지와 힘, 그리고 불확실성
        self.results = {
            "energy": energy_first_model,  # 첫 번째 모델의 에너지
            "forces": forces_first_model,  # 첫 번째 모델의 힘
            "energy_uncertainty": energy_uncertainty,  # 앙상블의 불확실성
        }
