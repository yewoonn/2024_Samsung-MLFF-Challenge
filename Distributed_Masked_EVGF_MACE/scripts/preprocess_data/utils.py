...
def save_normalization_statistics(trajectory, out_dir, energy_type="free_energy"):
    ...
    #     energy_type = "energy"
    # energies = np.array([atoms.get_total_energy() for atoms in trajectory]) 
    # energies_per_atom = np.array([atoms.get_total_energy()/atoms.get_forces().shape[0] for atoms in trajectory]) 
    energies = np.array([atoms.get_total_energy() for atoms in trajectory]) 
    energies_per_atom = np.array([atoms.get_total_energy()/atoms.get_forces().shape[0] for atoms in trajectory]) 
